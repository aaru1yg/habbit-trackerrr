...
During validation, PR #14 independently landed at **12:48 UTC** as `26e737f00724f3090917e649a1d2d2af9b519284`: it included the original five commits plus brand/art completion (`594be5a`) and a first-class Goals implementation (`bf1407f`). This newer `main` was merged into the shipping branch rather than overwritten. Its goal model, reducer, UI, tests, icons, metadata and new artwork are preserved. Validation was rerun on the combined result.

Integrating it exposed a release-blocking omission: the existing cloud merge/has-data helpers did not include the new `goals` collection. A goal-only cloud document could be treated as empty, and “Merge both” could lose the other device’s goals. The minimal fix adds goals to the existing id-keyed merge, migration counts and data-presence contract, with unit/migration and real cross-browser goal-persistence checks. No auth provider, RLS policy or database architecture was changed. Aurora was also missing from the import/reload theme allowlist and is now preserved. Once the audit exercised the actual Aurora palette instead of silently falling back to Midnight, two low-contrast text tokens were found and corrected; the audit now asserts the requested theme really rendered.
...
- Preserved the full artwork delivered by the concurrent merge, including the previously missing achievements empty-state asset.
...
- Added focused regression/safety tests, expanded the layout/contrast audits to new and detail routes, made audit failures exit nonzero, and enforced 44×44 targets (with subpixel rounding tolerance).
...
`supabase/**`, auth providers, and the sync state machine remain unchanged. Only the existing collection merge/count contract and migration count display were extended for goals. Test-account secrets are consumed only inside GitHub Actions. No service-role key is required.
...
- The concurrently shipped first-class Goals model and expanded art library are preserved, not reimplemented. No next-version feature campaign or redesign was started.
