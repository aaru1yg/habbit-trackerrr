...
console.log('\n— Week / Insights / Mind (mobile) —')
{
  ...
  check('trend range has 4 options', await page.evaluate(() => document.querySelectorAll('[aria-label="Trend range"] .vseg-btn').length === 4))
  check('insights has an Overview / Deep dive switch', await page.evaluate(() => document.querySelectorAll('[aria-label="Insights view"] .vseg-btn').length === 2))
  check('trend defaults to 30D', await page.evaluate(() => document.querySelector('[aria-label="Trend range"] .vseg-btn.active')?.textContent === '30D'))
  ...
  await sleep(300)
  check('trend switches to 7D', await page.evaluate(() => document.querySelector('[aria-label="Trend range"] .vseg-btn.active')?.textContent === '7D'))
  ...
  await sleep(300)
  check('trend switches to 90D', await page.evaluate(() => document.querySelector('[aria-label="Trend range"] .vseg-btn.active')?.textContent === '90D'))
  await clickByText(page, '1Y', 'button')
  await sleep(300)
  check('trend switches to 1Y', await page.evaluate(() => document.querySelector('[aria-label="Trend range"] .vseg-btn.active')?.textContent === '1Y'))
...
console.log('\n— Calendar range modes —')
{
  ...
  const activeMode = () => page.evaluate(() => document.querySelector('#calendar-screen .vseg-btn.active')?.textContent || '')

  check('calendar has 3 range modes', await page.evaluate(() => document.querySelectorAll('#calendar-screen .vseg-btn').length === 3))
