/* V3 FINAL VISUAL QA — the closing browser proof for the V3 UI.
 *
 * Real production build (vite preview), real persisted fixture data
 * (seededStateV4 — no mock dashboards), real Chromium. No deployment from
 * here. One CI job per viewport (390×844, 430×932, 1440×900) captures the
 * fifteen V3 surfaces, runs the visual check battery on each (console/page
 * errors, document + element overflow, text-on-text overlap, clipped text,
 * 44px targets on mobile, broken assets), verifies the V3-specific systems
 * (HabitCard/HabitRing hierarchy, multi-series WorkCapacitySeries with
 * legend/toggle/tooltip/accessible summary, Goal Detail pace Actual vs
 * Expected vs Required, Omni, forms, sheets, dialogs, keyboard focus,
 * reduced motion) and writes results.json so the counts are publishable
 * through the Checks API.
 *
 *   Local / CI preview:
 *     V3_QA_VIEWPORT=390x844 node qa/v3-final-visual.mjs http://localhost:4173
 */
...
const metadata = { commit, version: null, target: `${base}/`, mode: 'local', viewports: [], results }
...
try {
  ...
  console.log(`Real browser: ${metadata.version}; commit: ${commit}; target: ${base}/`)
