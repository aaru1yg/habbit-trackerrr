...
export default function AdaptiveCommandCenter({ data, onComplete }) {
  ...
  return <>
    {next && <Reveal as="section" variant="up" className="card pad adaptive-next sp-depth lane-left" aria-label="Next best action">
      ...
      <div className="adaptive-next-main today-next-main">
        <div><p className="eyebrow">Consider</p><h2>{selected.label || selected.name}</h2><p className="adaptive-reason today-reason">{next.reason}</p></div>
        <div className="adaptive-meta today-meta"><span>Estimated: <strong>{fmt(next.estimatedMin)}</strong></span><span>Deadline: <strong>{next.deadline ? new Date(next.deadline).toLocaleString([], { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }) : 'None set'}</strong></span></div>
      </div>
      <div className="adaptive-actions today-actions"><button className="btn primary sm" onClick={() => onComplete(selected)}>Complete</button><Link className="btn ghost sm" to={hrefFor(selected)}>View</Link></div>
      <details className="adaptive-why today-why"><summary>Why this is prioritized</summary><div className="adaptive-signals"><span>Deadline: {selected.deadline ? new Date(selected.deadline).toLocaleDateString() : 'Not set'}</span><span>Progress: {next.progress == null ? 'Not enough data yet.' : `${next.progress}%`}</span><span>Remaining: {fmt(next.remainingMin)}</span><span>Priority: {selected.priority || 'Not set'}</span><span>Risk: <Risk risk={next.risk} /></span></div></details>
    </Reveal>}
    ...
    <Reveal as="section" variant="depth" className="card pad adaptive-capacity"><CardHead title="Today’s capacity"><span className="tiny muted">workload context</span></CardHead><div className="adaptive-capacity-grid today-capacity-grid"><span>Available<strong>{fmt(workload.availableMin)}</strong></span><span>Committed<strong>{workload.committedMin ? fmt(workload.committedMin) : 'Not enough data yet.'}</strong></span><span>Remaining<strong>{workload.remainingMin == null ? 'Not enough data yet.' : fmt(workload.remainingMin)}</strong></span></div>{workload.overloaded && <p className="pace-note" data-tone="warn">{workload.reason} Review: {priorities.map((p) => p.item.label || p.item.name).join(', ')}.</p>}</Reveal>
  </>
}
