...
const state = { profile: { dailyCapacityMin: 120 }, assignments: [{ id: 'a', name: 'Urgent', priority: 'high', progress: 0, estimateMin: 90, deadline: '2026-09-08T12:00:00' }], projects: [], goals: [], habits: [], checkins: {} }
describe('adaptive planning', () => {
 ...
 it('does not fabricate capacity', () => expect(buildDayPlan({ ...state, profile: {} }, { now }).fit).toBe('INSUFFICIENT DATA'))
 it('flags overload instead of pretending it fits', () => expect(buildDayPlan({ ...state, profile: { dailyCapacityMin: 60 } }, { now }).fit).toBe('OVERLOADED'))
